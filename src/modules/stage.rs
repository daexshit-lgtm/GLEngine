use std::{fs::read_to_string, rc::Rc as StdRc, vec};
use glam::Vec2;
use rclite::Rc;
use anyhow::{anyhow, Result};
use rustc_hash::FxHashMap;
use glium::{
    Blend, BlendingFunction, Depth, DepthTest, DrawParameters, Frame,
    LinearBlendingFactor, Program, backend::Context,
};
use crate::modules::{
    imgui::{UI2D, Window2D},
    render_3d::{
        aabb::Aabb,
        camera::Camera,
        model::{MCacheP, MMaterial, Model, ModelCache, SubMesh, Vertex, new_buffers},
        transform::Transform,
    },
    texture::Texture,
};
...
impl<'a> Stage<'a> {
    /// Creates a Stage with depth-tested, alpha-blended draw parameters.
    ...
    /// Loads a model from `assets/models/{path}` (cached).
    #[allow(unused)]
    ...
    /// Returns (and caches) a compiled shader from `assets/shaders/{shader}.{vert,frag}`.
    pub fn get_shader(&mut self, shader: &str) -> Rc<Program> {
        let key = format!("assets/shaders/{shader}");
        if !self.shader_cache.contains_key(&key) { self.cache_shader(&key).unwrap(); }
        Rc::clone(&self.shader_cache[&key])
    }

    /// Uploads a raw mesh without model-file caching.
    pub fn add_mesh(
        &mut self,
        vertices:   Vec<Vertex>,
        indices:    Vec<u32>,
        ...
        self.models.push(Model {
            ...
            shader_p,
            cache: MCacheP::Unique(ModelCache { sub_meshes, materials, bounds, v_buffer, i_buffer }),
        });
        Ok(self.models.len() - 1)
    }

    /// Returns (and caches) a ModelCache from `assets/models/{path}`.
    pub fn get_model_cache(&mut self, path: &str, dynamic: bool) -> Rc<ModelCache> {
        let key = format!("assets/models/{path}");
        if !self.model_cache.contains_key(&key) { self.cache_model(&key, dynamic).unwrap(); }
        Rc::clone(&self.model_cache[&key])
    }
    ...
    #[allow(unused)]
    /// Returns a copy of the cached AABB for `path`.
    ...
    #[allow(unused)]
    /// Replaces GPU buffers for a dynamic (unique) mesh.
    ...
    pub fn draw_models(&mut self, frame: &mut Frame) -> Result<()> {
        for model in &self.models {
            model.draw(frame, &self.cam, &self.draw_params)?;
        }
        Ok(())
    }
}
